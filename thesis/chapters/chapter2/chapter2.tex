...
In this case, we need to set up an OpenSSL server to receive the reverse shell on the attacker's side. This is done using the following commands:

\getcmdline{\commandpath{openssl-listener.txt}}{*}{openssl}{}

The first command generates a self-signed X.509 certificate and a new RSA 4096-bit key. Then, it saves the key and the certificate to specified \texttt{.pem} files. The \texttt{-noenc} option makes the private key not encrypted (no password is required to access it later).

The second command runs \texttt{openssl} in server mode for listening on a specified port. The certificate and key are loaded from the \texttt{.pem} files created with the first command, and the \texttt{-quiet} option will suppress the printing of session and certificate information. \cite{openssl-doc}
...
\getcmdline{\commandpath{curl-listener.txt}}{*}{from,import,class,def,self}{ShellHandler,do_GET,do_POST}
...
\getcmdline{\commandpath{curl.txt}}{*}{curl}{}
...
\dpd{additional dynamic libraries (apart from those listed with \texttt{ldd})\footnote{should be located in \texttt{/lib/} or \texttt{/lib/julia/}}
...
and module \texttt{Sockets} (files \texttt{Sockets.jl} and \texttt{Sockets/Project.toml}\footnote{usually located in \texttt{/usr/local/share/stdlib/<version>}})
}
