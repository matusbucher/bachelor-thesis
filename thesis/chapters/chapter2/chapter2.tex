...
The first command generates a self-signed X.509 certificate and a new RSA 4096-bit key. Then, it saves the key and the certificate to specified \texttt{.pem} files. The \texttt{-noenc} option just makes the private key not encrypted (no password is required to access it later).
